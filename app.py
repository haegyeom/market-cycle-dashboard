...
st.markdown("""
본 대시보드는 단순한 주가 추종을 넘어 **산업의 맥박, 글로벌 혈류, 탐욕의 온도, 이성적 중력**이라는 4대 축을 통해 
...
def load_data():
    try:
        data_map = {
            'Truck_Sales': 'HTRUCKSSAAR',      # 산업의 맥박 (K Units)
            'JPY_Rate': 'IRSTCI01JPM156N',     # 글로벌 혈류 (%)
            'Margin_Debt': 'BOGZ1FL663067003Q',# 탐욕의 온도 ($ Millions)
            'GDP': 'GDP'                       # 이성적 중력 ($ Billions)
        }
...
if df is not None:
    ...
    with st.expander("💡 대시보드 활용 및 인터랙티브 분석 가이드", expanded=True):
        ...
        **1. 포커스 분석 (Legend Toggle)**
        ...
        - **활용 예:** 'S&P 500'과 '탐욕의 온도(빚투)'만 켜두고 대조해 보면, 현재의 상승이 실물 경제에 기반한 것인지 아니면 과도한 레버리지에 의한 버블인지 직관적으로 파악할 수 있습니다.
    ...
    names = {
        'SP500': '성적표 (S&P 500)', 
        'Buffett_Indicator': '이성적 중력 (버핏 지수)', 
        'Margin_Debt': '탐욕의 온도 (빚투)', 
        'Truck_Sales': '산업의 맥박 (트럭 판매)', 
        'JPY_Rate': '글로벌 혈류 (일본 금리)'
    }
    ...
    fig_sub.add_trace(go.Scatter(x=df.index, y=df['Margin_Debt'], name="빚투", line=dict(color=colors['Margin_Debt'], width=2)), row=3, col=1)
    fig_sub.add_trace(go.Scatter(x=df.index, y=df['Truck_Sales'], name="트럭 판매", line=dict(color=colors['Truck_Sales'], width=2)), row=4, col=1)
    ...
    display_df.columns = ['S&P 500 ($)', '버핏 지수 (Ratio)', '빚투 ($M)', '트럭 판매 (K)', '일본 금리 (%)']
